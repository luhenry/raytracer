...
public class Program {
  ...
  public static void main(String[] args) {
    ...
    for (int j = image_height-1; j >= 0; --j) {
      err.printf("Scanlines remaining: %d\n", j);
      for (int i = 0; i < image_width; ++i) {
        Color pixel_color = new Color(0, 0, 0);
        for (int s = 0; s < samples_per_pixel; ++s) {
          double u = ((double)i + rand.nextDouble(0.0, 1.0)) / (image_width-1);
          double v = ((double)j + rand.nextDouble(0.0, 1.0)) / (image_height-1);
  ...
  private static Color ray_color(Ray ray, Hittable world, int depth) {
